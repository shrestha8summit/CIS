...
def perform_dh():
    ...
    pyperclip.copy(public_key_bytes)
    messagebox.showinfo("Copied", "Public key copied to clipboard!")

def show_ecc_options():
    clear_menu()
    tk.Button(menu_frame, text="Sign File", command=sign_file_with_ecc).grid(row=0, column=0, padx=5)
    tk.Button(menu_frame, text="Back", command=show_main_menu).grid(row=1, column=0, padx=5)

def sign_file_with_rsa():
    file_path = filedialog.askopenfilename()
    if not file_path:
        return
    ...
    if 'rsa_private_key' not in globals():
        messagebox.showerror("Key Error", "No RSA private key available. Please encrypt a message first.")
        return

    signature = rsa_sign(rsa_private_key, file_data)
    signed_file_path = f"Sig_{file_path.split('/')[-1]}.sig"

    with open(signed_file_path, 'wb') as file:
        file.write(signature)

    messagebox.showinfo("File Signed", f"Signed file saved as: {signed_file_path}")

def sign_file_with_ecc():
    file_path = filedialog.askopenfilename()
    if not file_path:
        return
    ...
    private_key, signature = ecc_sign(file_data)
    signed_file_path = f"Sig_{file_path.split('/')[-1]}.sig"

    with open(signed_file_path, 'wb') as file:
        file.write(signature)

    messagebox.showinfo("ECC Signed", f"ECC signed file saved as: {signed_file_path}")
...
def show_rsa_options():
    ...
    tk.Button(menu_frame, text="Sign File (RSA)", command=sign_file_with_rsa).grid(row=2, column=0, padx=5)
    tk.Button(menu_frame, text="Back", command=show_main_menu).grid(row=2, column=1, padx=5)
